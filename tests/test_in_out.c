...
#define N 32
uint8_t actual_out_frame[N][256];
uint8_t actual_out_frame_len[N];

static int test_number = 0;
const int test_count = sizeof(in_frames) / sizeof(uint16_t*);
...
int main(void)
{
	...
	while (test_number < test_count) {
		...
		modbus_process_msg(in_frames[test_number], frame_len[test_number]);
